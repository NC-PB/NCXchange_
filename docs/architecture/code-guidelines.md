...
- Comparison rules for NC programs, a source against the program compiled from it (implementation 13, P3-07; `tests/Ncx.Acceptance/NcComparer.cs`): two NC programs are equivalent when, after removing block numbers, comments and blank lines, trimming whitespace, and formatting every number of both sides with the target machine's `[format] decimals` (so `70.` and `70.0` and `70` are equal, and `-.534` equals `-0.534`), the remaining lines are equal in order; the header block of the source may carry modal words in another order than the compiled one, so a header line is compared as a set of words; a `TOOL CALL` or `T M6` line is compared as a set of words; the `program_end` line is not a difference where the source ends in another form or in none (D49). Everything else, including the order of words inside a motion block, must match. A difference the documents cannot settle is listed with the question it waits on, never passed over silently, and any other difference fails the test with a unified diff.
