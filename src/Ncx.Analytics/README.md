...
5. `Segments/`: the segment length, and `ToolVectors/`: the tool vector change with the convention of implementation 14 until the kinematics module exists (P4-03). They share `ValueStatistics.cs` and `VerbStatistics.cs` (the minimum, maximum and mean per verb), `Histogram.cs` (the configurable bins), `WorstMotions.cs` with `WorstMotion.cs` (the ten worst motions with their lines) and `MotionPositions.cs` with `PositionGap.cs` (why a position makes no known change).

The tests are in `../../tests/Ncx.Acceptance/Analytics/` until the project has enough to deserve its own (implementation 00-method 4). The analytics report in their text and raise no diagnostics of their own, so the project has no `DiagnosticCodes` class yet; its codes are `ANA` (D98).
