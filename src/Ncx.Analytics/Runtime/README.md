...
- `MovingAxes.cs`: the axes a motion moves (on an arc both plane axes, a full circle included, virtual machine 3.2), and the smallest rapid, max_feed and acceleration among them (virtual machine 8); rotary travel belongs to the kinematics module (virtual machine 9), except in the polar and cylinder plane, where the C word is a length (D102).
...
- `MachineDynamics.cs`: `[[axis]]` rapid, max_feed, acceleration, `[dynamics]` block_time, path_mode, corner_speed (machine-config 4), accel_time of the spindle tables (machine-config 5), and the tables of the report that list them, the linear and the rotary axes apart; without `[dynamics]` the estimate is distance over feed.
