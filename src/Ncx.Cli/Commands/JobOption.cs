...
/// <summary>
/// --job &lt;name.ncxjob.toml&gt; of ncx check and ncx analyze: a job manifest in place of the file, whose channel
/// programs run as one job in rounds, on the machine the manifest names unless --machine names another
/// (machine-config 8; virtual machine 3.7; architecture 10, F24). The file and --job exclude each other, and one of
/// them is required.
/// </summary>
internal static class JobOption
{
    ...
    /// </summary>
    /// <param name="command">check or analyze.</param>
    ...
    public static Option<string> AddTo(Command command, Argument<string> file)
    {
        ...
        {
            Description = "A job manifest, <name>.ncxjob.toml, in place of the file: the programs of its channels run "
                + "as one job, on the machine the manifest names unless --machine names another.",
