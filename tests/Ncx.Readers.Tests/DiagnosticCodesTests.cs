...
/// <summary>
/// The codes of every part of the DiagnosticCodes class of Ncx.Readers follow D98, so that parts written side by side
/// cannot clash.
/// </summary>
public sealed class DiagnosticCodesTests
{
    // A code is an area prefix and three digits; Ncx.Readers holds the area RDR (D98, implementation 00-method 4).
    [Fact]
    public void DiagnosticCodes_EveryCode_IsRdrWithThreeDigits()
    {
        foreach (FieldInfo field in CodeFields())
        {
            Assert.Matches("^RDR[0-9]{3}$", Code(field));
        }
    }
