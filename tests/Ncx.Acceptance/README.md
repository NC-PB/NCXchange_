...
End to end (implementation 00-method 4): the example files of the specification (`Examples/`), the rules of the repository itself (`Repository/`: the project references of architecture 3, the packages of code-guidelines 9, the embedded examples equal to their originals, the files the tour `../../docs/reading-the-code.md` names), and the tests of `Ncx.Cli` (`Cli/`), `Ncx.Analytics` (`Analytics/`) and `Ncx.Plugins` (`Plugins/`, with the two plugins they load in `Plugins/TestPlugins/`) until they have test projects of their own. `Jobs/` runs the Nakamura WY-250L pair as one job (milestone M9) with the manifest of `Fixtures/`, the inputs of this project that are no example of the specification. `Expected/` holds the whole-file outputs the example tests compare with, `Expected/analytics/` the reports that are the reference of the analytics.
