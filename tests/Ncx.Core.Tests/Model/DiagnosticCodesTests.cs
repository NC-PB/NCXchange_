...
/// <summary>
/// The codes of every part of DiagnosticCodes follow D98, so that parts written side by side cannot clash.
/// </summary>
public sealed class DiagnosticCodesTests
{
    // A code is an area prefix and three digits; Ncx.Core holds the areas PAR and VM (D98, implementation
    // 00-method 4).
    [Fact]
    public void DiagnosticCodes_EveryCode_IsParOrVmWithThreeDigits()
    {
        foreach (FieldInfo field in CodeFields())
        {
            Assert.Matches("^(PAR|VM)[0-9]{3}$", Code(field));
        }
    }
