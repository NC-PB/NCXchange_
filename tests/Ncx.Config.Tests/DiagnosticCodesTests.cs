...
/// <summary>
/// The codes of every part of the DiagnosticCodes class of Ncx.Config follow D98, so that parts written side by side
/// cannot clash.
/// </summary>
public sealed class DiagnosticCodesTests
{
    // A code is an area prefix and three digits; Ncx.Config holds the area CFG (D98, implementation 00-method 4).
    [Fact]
    public void DiagnosticCodes_EveryCode_IsCfgWithThreeDigits()
    {
        foreach (FieldInfo field in CodeFields())
        {
            Assert.Matches("^CFG[0-9]{3}$", Code(field));
        }
    }
    ...
    // Every constant of every part of the class.
