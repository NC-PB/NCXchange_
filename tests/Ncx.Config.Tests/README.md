...
The tests of `src/Ncx.Config`: a machine file loaded table by table with every mistake reported on its line (`MachineConfigLoaderTests`, machine-config 1 to 9), the eight machine files of the repository (`ExampleMachinesTests`: the five examples of machine-config 11 and the three mills of `machines/`), the default machine (`DefaultMachineTests`, D103), the job manifest and the vars file (machine-config 8), the spelling of M and G codes (`FunctionValuesTests`, D105), and the nearest key a WARNING names (`NearestKeyTests`). `Templates/` and `Cycles/` mirror their source folders.
